...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Calculate subtotal based on duration days
    const calcSubtotal = items.reduce((sum, item) => {
      const daysMatch = item.duration ? item.duration.match(/(\d+)\s*day/) : null;
      const days = daysMatch ? parseInt(daysMatch[1]) : 1;
      return sum + (parseFloat(subtotal) / items.length) * days;
    }, 0);
    const displaySubtotal = parseFloat(subtotal);
    const displayTax = parseFloat(tax);
    const displayTotal = parseFloat(total);

    const firstItem = items[0] || {};
    const manageUrl = `https://backdropbarco.com/manage.html?order=${orderNumber}&name=${encodeURIComponent(firstItem.title||'')}&variant=${encodeURIComponent(firstItem.variant||'')}&qty=${firstItem.qty||1}&pickup=${encodeURIComponent(firstItem.pickup||'')}&ret=${encodeURIComponent(firstItem.ret||'')}&total=${encodeURIComponent(parseFloat(total).toFixed(2))}&email=${encodeURIComponent(customerEmail)}&customer=${encodeURIComponent(customerName)}`;
...
<table width="580" cellpadding="0" cellspacing="0" style="max-width:580px;width:100%;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:white;border:1px solid rgba(201,180,154,0.25);border-radius:10px;overflow:hidden;">
      <tr><td style="padding:12px 24px;border-bottom:1px solid rgba(201,180,154,0.15);">
        <table width="100%"><tr>
          ...
          <td style="text-align:right;font-size:13px;color:#3a2e28;">$${displaySubtotal.toFixed(2)}</td>
        </tr></table>
      </td></tr>
      <tr><td style="padding:12px 24px;border-bottom:1px solid rgba(201,180,154,0.2);">
        <table width="100%"><tr>
          ...
          <td style="text-align:right;font-size:13px;color:#3a2e28;">$${displayTax.toFixed(2)}</td>
        </tr></table>
      </td></tr>
      <tr><td style="padding:12px 24px;border-bottom:1px solid rgba(201,180,154,0.2);">
        <table width="100%"><tr>
          <td style="font-size:14px;font-weight:400;color:#3a2e28;">Order Total</td>
          <td style="text-align:right;font-size:14px;font-weight:400;color:#3a2e28;">$${displayTotal.toFixed(2)}</td>
        </tr></table>
      </td></tr>
      <tr><td style="padding:14px 24px;background:#f5ede1;">
        <table width="100%"><tr>
          <td style="font-size:12px;color:#7a6a60;">Total Charged Today</td>
          <td style="text-align:right;font-size:17px;font-weight:400;color:#3a2e28;">$${displayTotal.toFixed(2)}</td>
        </tr></table>
      </td></tr>
    </table>
  ...
  <tr><td style="background:#fdf8f3;padding:0 40px 28px;">
    ...
    <div style="font-size:12px;color:#7a6a60;line-height:2;margin-bottom:16px;">
    ...
    <a href="${manageUrl}" style="display:inline-block;padding:11px 24px;background:none;color:#a32d2d;text-decoration:none;border-radius:8px;font-size:10px;letter-spacing:0.18em;text-transform:uppercase;border:1px solid rgba(163,45,45,0.35);">Cancel Reservation</a>
  </td></tr>
